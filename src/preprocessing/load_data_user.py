...
기존 ``load_data.py``의 TODO 원본은 보존하고, 사용자 담당 구현을 이 파일에
분리한다. 이 모듈은 텍스트 정제나 Document 생성을 수행하지 않는다.
"""
...
def normalize_festival(row: dict[str, Any]) -> dict[str, Any]:
    ...
    normalized["contentid"] = str(normalized["contentid"])
    return normalized
...
def build_info_lookup(
    ...
    *,
    ...
    for row_index, row in enumerate(info_rows):
        wrapper_id = str(row.get("contentid", ""))
        error = row.get("error")
        ...
        for detail in details:
            content_id = str(detail.get("contentid", ""))
            if not content_id.strip():
                if skipped_rows is not None:
                    skipped_rows.append(
                        {
...
def _validate_row_list(
    ...
    *,
    ...
    for row_index, row in enumerate(value):
        ...
        if not str(row["contentid"]).strip():
            raise ValueError(
