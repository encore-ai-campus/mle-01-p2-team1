...
- LLM 기반 Entity·Relation 추출과 Entity Resolution을 결합해 축제 지식그래프를 구축했습니다.
- Neo4j Aura와 Streamlit을 연결해 그래프 탐색과 챗봇 검색을 실제로 시연했습니다.

**Problem**

- 초기에는 검색 라우팅과 Text2Cypher 검증이 부족해 잘못된 답변과 Cypher 오류가 발생했습니다.
- UI 수정 과정에서 배포 버전과 로컬 버전의 차이를 확인하는 데 시간이 걸렸습니다.

**Try**

- 질문 유형별 라우팅과 Cypher 검증을 체계화해 검색 정확도를 높이겠습니다.
- 배포 전 자동 테스트와 화면별 체크리스트를 마련하겠습니다.

### 현유진

**Keep**

- 역할을 세분화해 분담하면서 프로젝트 개념과 구현에 집중할 수 있었습니다.
- 각자의 결과물을 연결하며 실제 협업 과정을 경험했습니다.

**Problem**

- 맡지 않은 부분의 코드와 작업 내용을 바로 이해하기 어려웠습니다.
- 개별 작업이 전체 결과물과 어떻게 연결되는지 파악하는 데 어려움이 있었습니다.

**Try**

- 팀원들의 작업 내용까지 확인하며 프로젝트 전체 흐름을 파악하겠습니다.
- 기능 간 연결 구조를 정리해 전체 아키텍처를 이해하겠습니다.

### 조자룡

**Keep**

- 온톨로지 설계와 품질 평가, 골든셋 구축을 팀원들과 함께 검토했습니다.
- 다양한 관점으로 기준을 보완하고 애매한 문제를 빠르게 합의했습니다.

**Problem**

- 설계 초기의 기준 문서와 추출 예시·반례가 충분히 구체적이지 않았습니다.
- 검수 중간에 Entity·Relation 기준을 다시 합의해야 하는 경우가 있었습니다.

**Try**

- 온톨로지 설계 전에 Competency Question을 5~10개 작성하겠습니다.
- Entity, Relation, Key, Evidence, Source 규칙을 문서에 명확히 기록하겠습니다.

### 봉기람

**Keep**

- 원본 데이터를 보존하고 필요한 파일만 PR에 포함하는 작업 방식을 유지했습니다.
- 결과 형식뿐 아니라 관계와 출처 근거까지 검증했습니다.

**Problem**

- 기존 결과 검증과 새로운 LLM 추출을 명확히 구분하지 못한 부분이 있었습니다.
- 온톨로지·테스트·평가 기준 차이로 오류를 늦게 발견했습니다.

**Try**

- 작업 전 데이터·모델·온톨로지 버전과 출력 경로를 체크리스트로 확인하겠습니다.
- 구조 검증부터 근거·수동 검토·평가 호환성까지 순서대로 점검하겠습니다.

### 백관민

**Keep**

- 역할 분배와 Git 브랜치 활용을 프로젝트 전반에 걸쳐 잘 수행했습니다.
- 이전 프로젝트에서 배운 협업 방식을 다시 활용했습니다.

**Problem**

- Entity 수동 분류 과정에서 착각으로 데이터가 오염된 사례가 있었습니다.
- 챗봇 구현과 데이터 정제에 더 많은 시간이 필요했습니다.

**Try**

- 데이터 분류 시 확실하고 통일된 정제 방식을 적용하겠습니다.
- 챗봇 코드를 더 정밀하게 작성하고 데이터 정제에 충분한 시간을 투자하겠습니다.
